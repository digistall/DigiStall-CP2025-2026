...
(async () => {
  ...
  try {
    console.log('🔧 FIXING ALL MOBILE STORED PROCEDURES\n');

    // 1. Fix sp_getStallsByTypeForApplicant - correct parameter order
    console.log('1️⃣ Fixing sp_getStallsByTypeForApplicant (parameter order)...');
    ...
    // 2. Fix sp_getFullStallholderInfo - return individual fields for decryption
    console.log('2️⃣ Fixing sp_getFullStallholderInfo (individual fields for decryption)...');
    ...
    // 3. Fix sp_getStallholderStallsForDocuments
    console.log('3️⃣ Fixing sp_getStallholderStallsForDocuments...');
    ...
    // 4. Fix sp_getLatestApplicationInfo
    ...
    // 5. Fix getApplicantApplicationsDetailed
    console.log('5️⃣ Fixing getApplicantApplicationsDetailed...');
    ...
    // 6. Fix getAvailableStallsByApplicant
    console.log('6️⃣ Fixing getAvailableStallsByApplicant...');
    ...
    // 7. Fix sp_getStallholderPaymentHistory  
    console.log('7️⃣ Fixing sp_getStallholderPaymentHistory...');
    await conn.query('DROP PROCEDURE IF EXISTS sp_getStallholderPaymentHistory');
    await conn.query(`
      CREATE PROCEDURE sp_getStallholderPaymentHistory(
      ...
      BEGIN
        SELECT 
          ...
          b.branch_name
        FROM payment p
        INNER JOIN stall s ON p.stall_id = s.stall_id
        INNER JOIN section sec ON s.section_id = sec.section_id
        INNER JOIN floor f ON sec.floor_id = f.floor_id
        INNER JOIN branch b ON f.branch_id = b.branch_id
    ...
    console.log('\n✅ ALL MOBILE STORED PROCEDURES FIXED!');
