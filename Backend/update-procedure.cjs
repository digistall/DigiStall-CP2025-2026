...
async function updateProcedure() {
  ...
  try {
    ...
    await connection.execute('DROP PROCEDURE IF EXISTS getAllStallholdersDetailed');
    
    // Create the updated procedure that only shows stallholders WITH stalls
    const procedureSQL = `
...
END`;
    ...
    console.log('✅ Updated getAllStallholdersDetailed procedure successfully');
