...
export async function createCollector(req, res) {
  ...
  try {
    ...
    const [tableResult] = await connection.execute(
      "CALL sp_checkCollectorTableExists()"
    );
...
export async function terminateInspector(req, res) {
  ...
  try {
    ...
    await connection.execute("CALL sp_terminateInspector(?, ?)", [
      id,
...
export async function terminateCollector(req, res) {
  ...
  try {
    ...
    await connection.execute("CALL sp_terminateCollector(?, ?)", [
      id,
      reason || "Terminated by manager",
    ]);
  ...
  try {
    ...
    if (!firstName || !lastName || !email || !phoneNumber) {
      return res.status(400).json({
        success: false,
        message:
          "All fields are required: firstName, lastName, email, phoneNumber",
      });
    }
    ...
    if (!phoneRegex.test(phoneNumber)) {
      return res.status(400).json({
        success: false,
        message: "Invalid phone number format. Must be 09XXXXXXXXX",
      });
    }

    console.log(`📝 Updating collector ID: ${collectorId}`);

    connection = await createConnection();

    // Update collector using stored procedure
    const [result] = await connection.execute(
      "CALL sp_updateCollector(?, ?, ?, ?, ?)",
      [collectorId, firstName, lastName, email, phoneNumber]
    );
    ...
    console.log(`✅ Collector updated successfully: ${collectorId}`);
