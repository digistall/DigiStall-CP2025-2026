  ...
  try {
    ...
    if (!auctionInfo.length) {
      return res.status(404).json({
        success: false,
        message: 'No auction found for this stall'
      });
    }
    ...
    if (!application_id) {
      ...
      if (existingApp.length > 0) {
        ...
        finalApplicationId = appResult.application_id;
      }
    }
...
export const getAuctionParticipants = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to fetch auction participants'
    });
  } finally {
    if (connection) await connection.end();
  }
