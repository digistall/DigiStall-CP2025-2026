...
(async () => {
  ...
  try {
    console.log('🔧 CREATING VENDOR MODULE STORED PROCEDURES\n');
    console.log('========================================\n');

    // ========================================
    // 1. CREATE VENDOR WITH RELATIONS
    // ========================================
    console.log('1️⃣ Creating sp_createVendorWithRelations...');
    ...
    await conn.query(`
      ...
      BEGIN
        ...
        IF p_spouse_first_name IS NOT NULL THEN
          INSERT INTO vendor_spouse (
            ...
            p_spouse_first_name, p_spouse_age, p_spouse_birthdate,
        ...
        -- Create vendor_child if child data provided
        IF p_child_first_name IS NOT NULL THEN
          INSERT INTO vendor_child (
            first_name, age, birthdate, created_at, updated_at
          ) VALUES (
            p_child_first_name, p_child_age, p_child_birthdate, NOW(), NOW()
          );
      ...
      END
    `);
    console.log('✅ sp_createVendorWithRelations created successfully\n');

    // ========================================
    // 2. GET ALL VENDORS WITH RELATIONS
    // ========================================
    console.log('2️⃣ Creating sp_getAllVendorsWithRelations...');
    ...
    await conn.query(`
      ...
      END
    `);
    console.log('✅ sp_getAllVendorsWithRelations created successfully\n');

    // ========================================
    // 3. GET VENDOR BY ID WITH RELATIONS
    // ========================================
    console.log('3️⃣ Creating sp_getVendorWithRelations...');
    ...
    await conn.query(`
      ...
      END
    `);
    console.log('✅ sp_getVendorWithRelations created successfully\n');

    // ========================================
    // 4. UPDATE VENDOR WITH RELATIONS
    // ========================================
    console.log('4️⃣ Creating sp_updateVendorWithRelations...');
    ...
    await conn.query(`
      ...
      END
    `);
    console.log('✅ sp_updateVendorWithRelations created successfully\n');

    // ========================================
    // 5. DELETE/DEACTIVATE VENDOR
    // ========================================
    console.log('5️⃣ Creating sp_deleteVendorWithRelations...');
    ...
    await conn.query(`
      ...
      BEGIN
        ...
        IF p_hard_delete = TRUE THEN
          -- Hard delete: Remove vendor and related records
      ...
      END
    `);
    console.log('✅ sp_deleteVendorWithRelations created successfully\n');

    // ========================================
    // 6. GET VENDORS BY STATUS
    // ========================================
    console.log('6️⃣ Creating sp_getVendorsByStatus...');
    await conn.query('DROP PROCEDURE IF EXISTS sp_getVendorsByStatus');
    await conn.query(`
      CREATE PROCEDURE sp_getVendorsByStatus(IN p_status VARCHAR(20))
      BEGIN
        SELECT 
          v.vendor_id,
          v.first_name,
      ...
      END
    `);
    console.log('✅ sp_getVendorsByStatus created successfully\n');

    // ========================================
    // 7. SEARCH VENDORS
    // ========================================
    console.log('7️⃣ Creating sp_searchVendors...');
    await conn.query('DROP PROCEDURE IF EXISTS sp_searchVendors');
    await conn.query(`
      CREATE PROCEDURE sp_searchVendors(IN p_search_term VARCHAR(255))
      BEGIN
        SELECT 
          v.vendor_id,
          v.first_name,
        ...
        LEFT JOIN assigned_location al ON v.assigned_location_id = al.assigned_location_id
        WHERE 
          v.first_name LIKE CONCAT('%', p_search_term, '%') OR
          v.last_name LIKE CONCAT('%', p_search_term, '%') OR
          v.email LIKE CONCAT('%', p_search_term, '%') OR
          v.contact_number LIKE CONCAT('%', p_search_term, '%') OR
          v.vendor_identifier LIKE CONCAT('%', p_search_term, '%') OR
          vb.business_name LIKE CONCAT('%', p_search_term, '%')
        ORDER BY v.created_at DESC;
      END
    `);
    console.log('✅ sp_searchVendors created successfully\n');

    // ========================================
    // 8. GET VENDOR DOCUMENTS
    // ========================================
    console.log('8️⃣ Creating sp_getVendorDocuments...');
    await conn.query('DROP PROCEDURE IF EXISTS sp_getVendorDocuments');
    await conn.query(`
      CREATE PROCEDURE sp_getVendorDocuments(IN p_vendor_id INT)
      BEGIN
        SELECT 
          vd.vendor_document_id,
          vd.document_type_id,
          dt.type_name as document_type_name,
          dt.description as document_type_description,
          vd.document_name,
          vd.document_mime_type,
          vd.verification_status,
          vd.verified_by,
          vd.verified_at,
          vd.remarks,
          vd.submitted_at,
          LENGTH(vd.document_data) as document_size
        FROM vendor_documents vd
        INNER JOIN document_types dt ON vd.document_type_id = dt.document_type_id
        INNER JOIN vendor v ON vd.vendor_document_id = v.vendor_document_id
        WHERE v.vendor_id = p_vendor_id
        ORDER BY vd.submitted_at DESC;
      END
    `);
    console.log('✅ sp_getVendorDocuments created successfully\n');
    ...
    console.log('\n========================================');
    console.log('✅ ALL VENDOR STORED PROCEDURES CREATED SUCCESSFULLY!');
    console.log('========================================\n');

  } catch (error) {
    console.error('❌ Error creating vendor procedures:', error.message);
    console.error(error);
  } finally {
    await conn.end();
