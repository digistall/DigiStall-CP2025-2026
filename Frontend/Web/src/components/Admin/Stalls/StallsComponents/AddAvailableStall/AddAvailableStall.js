...
export default {
      ...
      try {
        ...
        if (this.requiresDuration) {
          ...
          deadlineDate.setDate(
            deadlineDate.getDate() + parseInt(this.newStall.deadlineDays)
          );
        ...
        console.log("- employeePermissions:", JSON.parse(sessionStorage.getItem("employeePermissions") || "[]"));
        ...
        if (result.success) {
          ...
          console.log(
            "Stall added successfully - will show real-time update after popup"
          );
  ...
  computed: {
    ...
    selectedFloorName() {
      ...
      const selectedFloor = this.floorOptions.find(
        (floor) => floor.value === this.newStall.floorId
      );
    ...
    requiresDuration() {
      return (
        this.newStall.priceType === "Raffle" || this.newStall.priceType === "Auction"
      );
    },
