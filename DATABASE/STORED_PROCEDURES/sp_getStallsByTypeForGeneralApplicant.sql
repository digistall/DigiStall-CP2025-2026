...
-- Updated: March 2, 2026 - Fixed JOIN paths to match actual schema
-- ============================================
-- IMPORTANT: The stall table uses section_id to join to section,
--            then section → floor → branch for the full hierarchy
-- ============================================
...
BEGIN
    ...
    -- Join path: stall → section → floor → branch
    SELECT 
        s.stall_id,
        s.stall_number,
        s.stall_location,
        ...
        CASE 
            WHEN EXISTS (
                ...
                AND a.application_status IN ('Pending', 'Approved', 'pending', 'approved')
            ) THEN 'applied'
            WHEN EXISTS (
