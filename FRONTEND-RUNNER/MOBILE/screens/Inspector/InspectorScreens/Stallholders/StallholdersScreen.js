...
const StallholdersScreen = ({ onSelectStallholder }) => {
  ...
  const loadStallholders = async () => {
    try {
      ...
      if (response.success) {
        console.log('Γ£à Stallholders loaded:', response.count);
        setStallholders(response.data || []);
      } else {
        console.error('Γ¥î Failed to load stallholders:', response.message);
        setError(response.message);
      }
    } catch (error) {
      console.error('Γ¥î Error loading stallholders:', error);
