...
export const getActiveAuctions = async (req, res) => {
  ...
  try {
    ...
    console.log("ðŸ” getActiveAuctions - User details:", {
      userType,
      userId,
    ...
    if (userType === "business_manager") {
      ...
      console.log("ï¿½ Employee permission check:", {
        permissions,
    ...
    console.log(`ðŸ“‹ Found ${auctions.length} auction stalls for ${userType} ${userId}`);
    ...
    console.error('âŒ Get active auctions error:', error);
    res.status(500).json({
      success: false,
...
export const getAuctionDetails = async (req, res) => {
  ...
  try {
    ...
    console.error('âŒ Get auction details error:', error);
    res.status(500).json({
      success: false,
