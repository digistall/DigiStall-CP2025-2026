...
export const selectAuctionWinner = async (req, res) => {
  ...
  try {
    ...
    console.error('âŒ Select auction winner error:', error);
    res.status(500).json({
      success: false,
    ...
    for (const auction of expiredAuctions) {
      try {
        ...
        if (winnerInfo.length === 0) {
          console.error(`âŒ No winning bid found for auction ${auction.auction_id}`);
          continue;
        }
        ...
        console.error(`âŒ Error processing auction ${auction.auction_id}:`, error);
        results.push({
          auctionId: auction.auction_id,
          stallNo: auction.stall_no,
          status: 'error',
