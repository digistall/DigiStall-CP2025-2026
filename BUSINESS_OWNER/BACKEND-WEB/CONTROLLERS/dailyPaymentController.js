...
const DailyPaymentController = {
  ...
  getAllDailyPayments: async (req, res) => {
    ...
    try {
      ...
      console.log("ðŸ” Fetching all daily payments");

      const [result] = await connection.execute("CALL getAllDailyPayments()");

      const payments = result[0] || [];
      console.log("ðŸ“Š Daily payments found:", payments.length);
      ...
      console.error("âŒ Error fetching daily payments:", error);
      res.status(500).json({
        success: false,
  ...
  getDailyPaymentById: async (req, res) => {
    ...
    try {
      ...
      console.log("ðŸ” Fetching daily payment with receipt ID:", parsedId);
      ...
      console.log("ðŸ“Š Daily payment found:", result[0][0]);
      ...
      console.error("âŒ Error fetching daily payment:", error);
      res.status(500).json({
        success: false,
  ...
  addDailyPayment: async (req, res) => {
    ...
    try {
      ...
      console.log("âž• Adding daily payment:", {
        collectorId,
        vendorId,
        amount,
        referenceNo,
      ...
      console.log(
        "ðŸ“‹ Raw stored procedure result:",
      ...
      console.log("ðŸ“¦ Stored procedure result:", paymentData);

      if (!paymentData.success) {
        console.log("âš ï¸ Payment creation failed:", paymentData.message);
        return res.status(400).json({
          success: false,
      ...
      console.log(
        "âœ… Daily payment added successfully:",
      ...
      console.error("âŒ Error adding daily payment:", error);
      res.status(500).json({
        success: false,
  ...
  updateDailyPayment: async (req, res) => {
    ...
    try {
      ...
      console.log("âœï¸ Updating daily payment:", receiptId);
      ...
      console.log("âœ… Daily payment updated successfully");
      ...
      console.error("âŒ Error updating daily payment:", error);
      res.status(500).json({
        success: false,
  ...
  deleteDailyPayment: async (req, res) => {
    ...
    try {
      ...
      console.log("ðŸ—‘ï¸ Deleting daily payment:", receiptId);
      ...
      console.log("âœ… Daily payment deleted successfully");
      ...
      console.error("âŒ Error deleting daily payment:", error);
      res.status(500).json({
        success: false,
  ...
  getAllVendors: async (req, res) => {
    ...
    try {
      ...
      console.error("âŒ Error fetching vendors:", error);
      res.status(500).json({
        success: false,
  ...
  getAllCollectors: async (req, res) => {
    ...
    try {
      ...
      console.log("ðŸ” Fetching all collectors");

      const [result] = await connection.execute("CALL getAllCollectors()");

      const collectors = result[0] || [];
      console.log("ðŸ“Š Collectors found:", collectors.length);
