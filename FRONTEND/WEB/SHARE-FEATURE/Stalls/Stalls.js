...
export default {
  ...
  methods: {
    ...
    handleRaffleWinnerSelected(winnerData) {
      ...
      // Close the modal since winner has been selected
      this.closeRaffleParticipantsModal()
      // Invalidate cache and force refresh to remove the now-occupied stall from the list
      dataCacheService.invalidatePattern('stalls')
    ...
    handleAuctionWinnerSelected(winnerData) {
      ...
      // Close the modal since winner has been selected
      this.closeAuctionParticipantsModal()
      // Invalidate cache and force refresh to remove the now-occupied stall from the list
      dataCacheService.invalidatePattern('stalls')
      this.fetchStalls(true)
    },
