...
DigiStall Team
    ...
    /**
     * Send employee password reset notification email (Manager-initiated)
     * Used when a manager resets an employee's password from the Employee Management page
     */
    async sendEmployeePasswordResetNotification(resetData) {
        try {
            const { email, employeeName, newPassword, resetBy } = resetData;

            const subject = `Your Password Has Been Reset - DigiStall`;
            
            const textContent = `
Hello ${employeeName || 'Employee'},

Your password has been reset by your manager in the Naga City Stall Management System.
...
- If you did not expect this password reset, contact your manager immediately

Reset by: ${resetBy || 'System'}
...
Stall Management Admin Team
            ...
            const htmlContent = `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <div style="padding: 30px; background: #f9f9f9;">
                        <p style="color: #333;">Hello <strong>${employeeName || 'Employee'}</strong>,</p>
                        <p style="color: #555;">Your password has been reset by your manager. Use the new password below to login:</p>
                        
                        <div style="background: white; padding: 25px; border-radius: 10px; margin: 25px 0; text-align: center; border: 2px dashed #1a237e;">
                            <p style="margin: 0 0 10px 0; color: #666; font-size: 14px;">Your New Password</p>
                            <h2 style="margin: 0; font-size: 28px; color: #1a237e; font-family: 'Courier New', monospace;">${newPassword}</h2>
                        </div>
                        ...
                        <div style="background: #f8d7da; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin: 20px 0;">
                            <p style="margin: 0; color: #721c24; font-size: 14px;">
                                <strong>⚠️ Security Reminder:</strong> Never share your password with anyone. Always log out when finished.
                            </p>
                        </div>
                        ...
                        <p style="color: #999; font-size: 12px;">
                            Reset by: ${resetBy || 'System'}<br>
                            Reset date: ${new Date().toLocaleString()}
                        </p>
                        
                        <p style="color: #333; margin-top: 25px;">Best regards,<br><strong>Stall Management Admin Team</strong></p>
                    </div>
                    <div style="background: #1a237e; color: white; padding: 15px; text-align: center; font-size: 12px;">
                        <p style="margin: 0;">© 2026 DigiStall - Naga City Stall Management System</p>
                    </div>
            ...
            console.log('✅ Employee password reset notification sent to:', email);
            return {
                success: true,
                messageId: result.messageId,
                recipient: email
            ...
            console.error('❌ Error sending employee password reset notification:', error);
            throw error;
        }
    }
}
