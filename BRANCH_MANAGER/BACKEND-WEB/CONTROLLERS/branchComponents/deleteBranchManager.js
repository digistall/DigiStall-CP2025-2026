...
export const deleteBranchManager = async (req, res) => {
  let connection;
  try {
    console.log('ðŸ—‘ï¸ Deleting branch manager - Request received');
    ...
    console.log('ðŸ” Delete validation:');
    ...
    console.log('ðŸ” Fetching manager data before deletion:', managerId);
    ...
    if (manager.branch_id) {
      console.log('ðŸ” Checking if this is the only manager for branch:', manager.branch_id);
      const [managerCount] = await connection.execute(
        'SELECT COUNT(*) as count FROM branch_manager WHERE branch_id = ?',
        [manager.branch_id]
      );

      console.log('ðŸ“Š Manager count for branch:', managerCount[0].count);
    ...
    console.log('ðŸ—‘ï¸ Deleting branch manager:', managerId);
    ...
    console.error('âŒ Delete branch manager error:', error);
