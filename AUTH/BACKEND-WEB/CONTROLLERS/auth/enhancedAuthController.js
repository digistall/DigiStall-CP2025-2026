  ...
  try {
    ...
    switch (userType) {
      ...
      case 'inspector':
        ...
        // Also update mobile_staff_sessions if exists
        try {
          await connection.execute(
            'UPDATE mobile_staff_sessions SET last_heartbeat = ? WHERE staff_id = ? AND staff_type = "inspector" AND is_active = 1',
            [philippineTime, userId]
          );
      ...
      case 'collector':
        ...
        try {
          await connection.execute(
            'UPDATE mobile_staff_sessions SET last_heartbeat = ? WHERE staff_id = ? AND staff_type = "collector" AND is_active = 1',
            [philippineTime, userId]
          );
        } catch (e) { /* Ignore if table doesn't exist */ }
        break;
