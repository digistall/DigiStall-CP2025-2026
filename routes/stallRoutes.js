...
import {
  createStallSchema, updateStallSchema, createRaffleSchema, joinRaffleSchema,
  selectWinnerSchema, extendTimerSchema, createAuctionSchema, placeBidSchema,
  uploadStallImageBlobSchema, uploadStallImagesBlobSchema, updateStallImageBlobSchema
} from '../middleware/schemas/stallSchemas.js'
...
router.post('/raffles/:stallId/create', viewOnlyForOwners, validate(createRaffleSchema), createRaffle)              // POST /api/stalls/raffles/:stallId/create - Create raffle for stall
router.post('/raffles/:stallId/join', viewOnlyForOwners, validate(joinRaffleSchema), joinRaffle)                  // POST /api/stalls/raffles/:stallId/join - Join raffle (triggers timer)
router.put('/raffles/:raffleId/extend', viewOnlyForOwners, validate(extendTimerSchema), extendRaffleTimer)         // PUT /api/stalls/raffles/:raffleId/extend - Extend raffle timer
router.put('/raffles/:raffleId/cancel', viewOnlyForOwners, cancelRaffle)              // PUT /api/stalls/raffles/:raffleId/cancel - Cancel raffle
router.post('/raffles/:raffleId/select-winner', viewOnlyForOwners, validate(selectWinnerSchema), selectRaffleWinner) // POST /api/stalls/raffles/:raffleId/select-winner - Select winner
...
router.post('/auctions/:stallId/create', viewOnlyForOwners, validate(createAuctionSchema), createAuction)             // POST /api/stalls/auctions/:stallId/create - Create auction for stall
router.post('/auctions/:stallId/bid', viewOnlyForOwners, validate(placeBidSchema), placeBid)                     // POST /api/stalls/auctions/:stallId/bid - Place bid (triggers timer)
router.put('/auctions/:auctionId/extend', viewOnlyForOwners, validate(extendTimerSchema), extendAuctionTimer)       // PUT /api/stalls/auctions/:auctionId/extend - Extend auction timer
...
router.post('/auctions/:auctionId/select-winner', viewOnlyForOwners, validate(selectWinnerSchema), selectAuctionWinner) // POST /api/stalls/auctions/:auctionId/select-winner - Confirm winner
...
router.post('/images/blob/upload', viewOnlyForOwners, validate(uploadStallImageBlobSchema), uploadStallImageBlob)            // POST /api/stalls/images/blob/upload - Upload single image as base64
router.post('/images/blob/upload-multiple', viewOnlyForOwners, validate(uploadStallImagesBlobSchema), uploadStallImagesBlob)  // POST /api/stalls/images/blob/upload-multiple - Upload multiple images as base64
...
router.put('/images/blob/:image_id', viewOnlyForOwners, validate(updateStallImageBlobSchema), updateStallImageBlob)          // PUT /api/stalls/images/blob/:image_id - Update image
