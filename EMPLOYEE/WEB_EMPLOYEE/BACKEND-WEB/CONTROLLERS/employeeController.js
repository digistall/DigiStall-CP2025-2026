...
export async function createEmployee(req, res) {
    ...
    try {
        ...
        console.log('ðŸ” Encrypting employee data before storage (email stays plain for login)...');
...
export async function getAllEmployees(req, res) {
    ...
    try {
        ...
        console.error('âŒ getAllEmployees - Error getting employees:', error);
...
export async function loginEmployee(req, res) {
    ...
    try {
        ...
        console.error('âŒ Employee login error:', error);
        res.status(500).json({ 
            success: false, 
...
export async function logoutEmployee(req, res) {
    ...
    try {
        ...
        if (employeeId) {
            ...
            console.log(`âœ… Logged out employee ${employeeId} at ${philippineTime}`);
        }
...
export async function getActiveSessions(req, res) {
    ...
    try {
        ...
        try {
            ...
            console.warn('âš ï¸ Could not fetch employee sessions:', empError.message);
        }
        ...
        try {
            ...
            console.log(`ðŸ“Š Found ${staffSessions.length} staff sessions, active: ${staffSessions.filter(s => s.is_active).length}`);
            if (staffSessions.length > 0) {
                console.log('ðŸ“Š Staff session sample:', JSON.stringify(staffSessions[0]));
            }
        } catch (staffError) {
            console.warn('âš ï¸ Could not fetch staff sessions:', staffError.message);
        }
