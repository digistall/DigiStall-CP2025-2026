...
-- ========================================
-- STEP 1: Create auction table if it doesn't exist
-- ========================================
CREATE TABLE IF NOT EXISTS `auction` (
  ...
  `auction_status` ENUM('Not Started', 'Waiting for Bidders', 'Active', 'Ended', 'Cancelled') DEFAULT 'Not Started',
  ...
  UNIQUE KEY `unique_auction_per_stall` (`stall_id`),
...
-- ========================================
-- STEP 2: Create auction_participants table
-- ========================================
...
-- Register participant for auction
...
-- ========================================
-- STEP 4: Insert sample auction data (OPTIONAL - for testing)
-- ========================================
-- Uncomment the lines below if you want to create sample auctions

-- INSERT INTO auction (stall_id, starting_price, auction_status, duration_hours)
-- SELECT stall_id, rental_price, 'Waiting for Bidders', 24
-- FROM stall
-- WHERE price_type = 'Auction'
-- AND NOT EXISTS (SELECT 1 FROM auction WHERE auction.stall_id = stall.stall_id)
-- LIMIT 5;
...
SELECT 
    table_name, 
    CASE 
        WHEN table_name IN ('auction', 'auction_participants') THEN '✅ EXISTS'
        ELSE '❌ MISSING'
    END as status
FROM information_schema.tables
WHERE table_schema = 'naga_stall' 
  AND table_name IN ('auction', 'auction_participants');
...
WHERE routine_schema = 'naga_stall'
  AND routine_name IN (
      'registerAuctionParticipant',
      'checkAuctionRegistration', 
      'getAuctionParticipants',
      'updateParticipantBid'
  );

SELECT '✅ AUCTION SYSTEM SETUP COMPLETE!' as final_status;
