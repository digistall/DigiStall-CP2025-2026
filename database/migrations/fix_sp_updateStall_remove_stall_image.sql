-- =============================================
-- Migration: Remove stall_image from sp_updateStall_complete
-- Date: 2025-12-08
-- Purpose: Fix the update stall procedure since stall_image 
--          column was removed from the stall table.
--          Images are now managed in stall_images table.
-- =============================================
...
CREATE PROCEDURE `sp_updateStall_complete` (
    ...
    IN `p_stall_image` VARCHAR(500),  -- Keep param for backward compatibility but don't use it
...
proc_label: BEGIN
    ...
    -- Update the stall (removed stall_image since it's now in stall_images table)
    UPDATE stall SET
        stall_no = p_stall_no,
...
-- Verification query
SELECT 'sp_updateStall_complete has been updated to remove stall_image column reference' AS migration_status;
