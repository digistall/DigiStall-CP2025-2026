...
export async function compressImage(filePath, options = {}) {
  ...
  try {
    ...
    const config = type === 'document' ? COMPRESSION_CONFIG.document 
                 : type === 'thumbnail' ? COMPRESSION_CONFIG.thumbnail 
                 : COMPRESSION_CONFIG;
    ...
    if (compressedSize < originalSize) {
      ...
      console.log(🗜️  Compressed: ${path.basename(filePath)} | ${formatSize(originalSize)} → ${formatSize(compressedSize)} (${savingsPercent}% saved));
 ...
 */
export async function compressBuffer(inputBuffer, mimeType = 'image/jpeg', options = {}) {
  ...
  try {
    ...
    if (compressedSize < originalSize) {
      const savingsPercent = ((1 - compressedSize / originalSize) * 100).toFixed(1);
      console.log(🗜️  Buffer compressed: ${formatSize(originalSize)} → ${formatSize(compressedSize)} (${savingsPercent}% saved));
...
export function compressUploads(options = {}) {
  return async (req, res, next) => {
    try {
      ...
      if (totalSaved > 0) {
        console.log(🗜️  Upload compression: ${imageFiles.length} files | ${formatSize(totalOriginal)} → ${formatSize(totalCompressed)} (saved ${formatSize(totalSaved)}));
      }
 ...
 */
export async function compressBase64Image(base64Data, options = { type: 'document' }) {
  try {
