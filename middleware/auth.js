...
// Alias for verifyToken (used by mobile routes)
const verifyToken = authenticateToken;

// Middleware to verify staff token and staff type (for mobile staff)
const verifyStaff = (allowedTypes = []) => {
  return (req, res, next) => {
    authenticateToken(req, res, (err) => {
      if (err) return;
      
      // Check if staff type is allowed
      if (allowedTypes.length > 0 && !allowedTypes.includes(req.user.staffType)) {
        return res.status(403).json({
          success: false,
...
// Named exports for both module styles
export {
  authenticateToken,
  authenticateUser,
